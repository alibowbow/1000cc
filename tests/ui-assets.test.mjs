...
test("순지 필사판·반응형 조선 서첩·무한 한자 찾기 v41이 오프라인 셸에 함께 연결된다", async function () {
  ...
  assert.match(app, /reason: "navigation"/);
  assert.match(app, /reason: "background"/);
  assert.match(app, /session\.pauseReason === "manual"/);
  ...
  assert.match(recognitionTheme, /@media \(min-width: 661px\) and \(max-width: 1024px\)/);
  assert.match(recognitionTheme, /@media \(prefers-reduced-motion: reduce\)/);
  assert.match(recognitionTheme, /#grid-session \.recognition-pause\s*\{[^}]*grid-column:\s*1;/s);
  assert.match(recognitionTheme, /\.recognition-pause\s*\{[^}]*place-self:\s*center;/s);
  assert.match(recognitionTheme, /\.recognition-pause h3\s*\{[^}]*white-space:\s*nowrap;/s);
  ...
  assert.match(recognitionTheme, /#grid-session:not\(\[hidden\]\)\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\);/s);
